...
Current software has satisfied the Level-8.0 source-engineering gate and additionally implements M6/M7 plus schema V2 without changing the caller-visible retrieval backend. The current production application image includes this software line, while M5–M7 behavioral actuation and data-dependent acceptance remain separately gated:
...
- Working Memory M5 provides exact task-scoped transient active context with explicit relevance, Salience-informed ordering, duplicate suppression, Token Governor limits, TTL/closeout, and delta emission. The code is present in the deployed application image, but M5 remains internal and is not exposed as a public production MCP state-management path;
- M6 Agent Self-Model provides trusted prospective evidence extraction, readiness, deterministic hypotheses, explicit review, accepted-only Knowledge Evolution persistence outside ordinary recall, and accepted-only bounded Working Memory advisory projection. The code is deployed, but persistent influence remains fail-closed without trusted/evidence readiness;
- M7 Memory Lifecycle provides deterministic shadow scoring, reversible suppression/reactivation, and access/lifecycle metadata. The code is deployed, but real-corpus suppression/reactivation remains gated on post-schema-V2 migration evidence;
- canonical schema V2 and stage-before-cutover migration/validation tooling are implemented in source, but this does not mean the currently deployed Qdrant corpus has been migrated.
...
Together, M3 + M4 + M5 satisfy the Level-8.0 **source** gate. The current deployed application image also contains M6 Agent Self-Model and M7 Memory Lifecycle plus schema-V2 metadata/migration tooling. M6 extracts trusted prospective Prediction/Outcome evidence, gates generation on at least 20 trusted resolved outcomes / 3 sessions / 3 topics / complete scan, generates deterministic bounded hypotheses, requires explicit review, persists accepted-only hypotheses through Knowledge Evolution outside ordinary recall, and can project accepted hypotheses into Working Memory as bounded advisory context. M7 provides deterministic shadow scoring, reversible soft suppression/reactivation, and access/lifecycle metadata without hard deletion or Knowledge-status mutation. The live Qdrant configuration still uses the pre-migration V1 stage collections, so this software deployment is not a schema-V2 data cutover and does not authorize M6/M7 real-corpus actuation.
