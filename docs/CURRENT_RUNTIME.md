...
Current local source has satisfied the Level-8.0 source-engineering gate and additionally implements M6/M7 plus schema V2 without changing the caller-visible retrieval backend. M5–M7 remain internal source capabilities whose production/runtime activation is separately gated:
...
- source-level M6 Agent Self-Model provides trusted prospective evidence extraction, readiness, deterministic hypotheses, explicit review, accepted-only Knowledge Evolution persistence outside ordinary recall, and accepted-only bounded Working Memory advisory projection;
- source-level M7 Memory Lifecycle provides deterministic shadow scoring, reversible suppression/reactivation, and access/lifecycle metadata. Real-corpus actuation remains gated on post-schema-V2 migration evidence;
